...
Может быть, если вы наберетесь смелости, вам даже не нужны pull request, но вам хочется, чтобы ваши 
зависимости коммитились в главную ветку? Именно об этом идет речь в этом видео.
...
Она работает практически с чем угодно.
...
И тогда любые pipeline, которые у меня есть, запустятся и создадут новый образ в этом случае, потому что Dockerfile изменился, и я буду жить долго и счастливо до следующего изменения.
...
А затем я перешел к различиям между тем, что было раньше и сейчас, сейчас и потом, и т. д. Только посмотрите на это. Это не простое изменение зависимости. Это действительно изменило одну из этих зависимостей. 

Но у этой зависимости есть и другие подзависимости, так что это изменило их все.
...
Но прежде чем я это сделаю, позвольте мне вытащить последнюю версию моего репозитория, потому что я только что создал pull request. Объединил их и т. д.

И я вижу, что теперь у меня есть последняя версия моего Dockerfile, а также go.mod и go.sum и я могу работать с Kubernetes.
Теперь вы можете подумать: «Как получилось так, что программа обновления обнаружила, что у меня есть Dockerfile и что у меня есть зависимости Go?».

Но это не так. Выясните, что существуют какие-то манифесты Kubernetes.

Почему я должен специально сообщать Renovate о том, что существуют манифесты Kubernetes? И ответ на этот вопрос относительно прост – манифесты Kubernetes могут быть практически любыми и находиться практически в любом месте.

Для Renovate было бы пустой тратой времени сканировать весь репозиторий на предмет наличия всех файлов yaml. Для того чтобы разобраться в этом, вовсе необязательно использовать язык yaml. Так что это было бы большой тратой времени впустую.
...
И это предложение относительно простое: «Образ было вот таким». Теперь должно получиться так, что если вы 
объедините этот pull request, то получите тег образа, который вы используете в манифесте развертывания Kubernetes.
...
Но существует потенциальная проблема. Потенциальный недостаток в том, что делает Renovate или в масштабах того, что делает Renovate. И я могу продемонстрировать это в одном из файлов моего манифеста Kubernetes.
Позвольте мне вывести данные на языке yaml.

Если вы не знакомы с Kubernetes, не являетесь экспертом, вы можете сказать: «Это выглядит нормально». Но это не так, потому что горизонтальный модуль автоматического масштабирования использует старую версию autoscaling/V2beta1. И прямо сейчас у нас есть autoscaling/v2. Нам следовало бы использовать версию autoscaling/v2, готовую к стабильному производству, а не autoscaling/V2beta1, которую я использую сам.
...
На самом деле речь идет не об управлении зависимостями Kubernetes, а только о самих образах.

А теперь я мог бы спросить: «Разве это считается зависимостью?». Являются ли версии Kubernetes чем-то таким, чем должна управлять Renovate или нет?

Библиотеки Go, которыми Renovate управляет, являются библиотеками сторонних производителей. Их па тоже является сторонней компанией. В каком-то смысле это так. Так что это может быть частью сферы применения или я не уверен. В любом случае было бы очень здорово, если бы компания Renovate расширила сферу своей деятельности за пределы того, чем он занимается прямо сейчас.
